...
void pomelo::publish_buyorder_if_needed(account_name account, asset bid, asset ask)
{
    ...
    // Validate bid symbol
    eosio_assert(bid.symbol == BOS, "Bid must be BOS.");

    // Validate ask symbol
    eosio_assert(ask.is_valid(), "Ask must be valid");
    // Validate ask symbol
    eosio_assert(ask.symbol != BOS, "Ask must be non-BOS");
...
{
    // Validate bid symbol
    eosio_assert(bid.symbol != BOS, "Bid must be non-BOS");

    // Validate ask symbol
    eosio_assert(ask.symbol == BOS, "Ask must be BOS");
...
void pomelo::buy(account_name account, asset bid, asset ask) 
{
    ...
    for (auto itr = unit_price_index.begin(); itr != unit_price_index.end();)
    {
        ...
        // Transfer BOS to seller  
             
        action(
            permission_level{ _self, N(active) },
            TOKEN_CONTRACT, N(transfer),
            make_tuple(_self, itr->account, asset(sold_eos, BOS), string("transfer")))
        .send();
...
void pomelo::sell(account_name account, asset bid, asset ask) 
{
    ...
    {    
        ...
        action(
            permission_level{ _self, N(active) },
            TOKEN_CONTRACT, N(transfer),
            make_tuple(_self, account, asset(sold_token * order_unit_price / PRICE_SCALE, BOS), string("transfer")))
        .send();
